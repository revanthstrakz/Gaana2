    ...
    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws GeneralSecurityException {
        ...
        try {
            ...
            if (bigInteger.signum() != -1) {
                if (bigInteger.compareTo(zza(curve)) == -1) {
                    ...
                    if (mod.equals(BigInteger.ZERO)) {
                        ...
                        if (zza.testBit(0) && zza.testBit(1)) {
                            bigInteger = mod.modPow(zza.add(BigInteger.ONE).shiftRight(2), zza);
                        } else if (zza.testBit(0) && !zza.testBit(1)) {
                            bigInteger = BigInteger.ONE;
                            a = zza.subtract(BigInteger.ONE).shiftRight(1);
                            int i = 0;
                            while (true) {
                                BigInteger mod2 = bigInteger.multiply(bigInteger).subtract(mod).mod(zza);
                                if (mod2.equals(BigInteger.ZERO)) {
                                    break;
                                }
                                BigInteger modPow = mod2.modPow(a, zza);
                                if (modPow.add(BigInteger.ONE).equals(zza)) {
                                    a = zza.add(BigInteger.ONE).shiftRight(1);
                                    BigInteger bigInteger2 = BigInteger.ONE;
                                    BigInteger bigInteger3 = bigInteger;
                                    for (int bitLength = a.bitLength() - 2; bitLength >= 0; bitLength--) {
                                        BigInteger multiply = bigInteger3.multiply(bigInteger2);
                                        bigInteger3 = bigInteger3.multiply(bigInteger3).add(bigInteger2.multiply(bigInteger2).mod(zza).multiply(mod2)).mod(zza);
                                        bigInteger2 = multiply.add(multiply).mod(zza);
                                        if (a.testBit(bitLength)) {
                                            multiply = bigInteger3.multiply(bigInteger).add(bigInteger2.multiply(mod2)).mod(zza);
                                            bigInteger2 = bigInteger.multiply(bigInteger2).add(bigInteger3).mod(zza);
                        ...
                        if (bigInteger != null && bigInteger.multiply(bigInteger).mod(zza).compareTo(mod) != 0) {
