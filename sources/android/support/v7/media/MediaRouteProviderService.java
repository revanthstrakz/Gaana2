...
public abstract class MediaRouteProviderService extends Service {
    ...
    public boolean updateCompositeDiscoveryRequest() {
        ...
        int i2 = i;
        while (i < size) {
            MediaRouteDiscoveryRequest mediaRouteDiscoveryRequest = ((ClientRecord) this.mClients.get(i)).mDiscoveryRequest;
            if (mediaRouteDiscoveryRequest != null && (!mediaRouteDiscoveryRequest.getSelector().isEmpty() || mediaRouteDiscoveryRequest.isActiveScan())) {
                i2 |= mediaRouteDiscoveryRequest.isActiveScan();
                if (obj == null) {
                    obj = mediaRouteDiscoveryRequest;
                } else {
                    if (builder == null) {
                        builder = new MediaRouteSelector.Builder(obj.getSelector());
                    }
                    builder.addSelector(mediaRouteDiscoveryRequest.getSelector());
                }
            }
            i++;
        }
        if (builder != null) {
            obj = new MediaRouteDiscoveryRequest(builder.build(), i2);
        }
        if (ObjectsCompat.equals(this.mCompositeDiscoveryRequest, obj)) {
            return false;
        }
        this.mCompositeDiscoveryRequest = obj;
        this.mProvider.setDiscoveryRequest(obj);
        return true;
    }
